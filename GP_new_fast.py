...
class GPStruct(object):
    def __init__(self,num_lbl,num_unlbl,train_batch_size,version,kernel_type):
        ...
        self.ART = ART(num_lbl,num_unlbl,train_batch_size,version,kernel_type)
        self.Fz_lbl = self.ART.Fz_lbl#torch.zeros((self.num_lbl,self.z_numchnls,self.z_height,self.z_width),dtype=torch.float32).cuda() #Feature matrix Fzl for latent space labeled vector matrix
        self.Fz_unlbl = self.ART.Fz_unlbl#torch.zeros((self.num_unlbl,self.z_numchnls,self.z_height,self.z_width),dtype=torch.float32).cuda() #Feature matrix Fzl for latent space unlabeled vector matrix
        self.ker_lbl = self.ART.ker_lbl#torch.zeros((self.num_lbl,self.num_lbl)).cuda() # kernel matrix of labeled vectors
        self.ker_lbl_ang = self.ART.ker_lbl_ang#torch.zeros((self.num_lbl,self.num_lbl)).cuda() # kernel matrix of labeled vectors
        self.ker_unlbl = self.ART.ker_unlbl#torch.zeros((self.num_unlbl,self.num_lbl)).cuda() # kernel matrix of unlabeled vectors
        self.ker_unlbl_ang = self.ART.ker_unlbl_ang#torch.zeros((self.num_unlbl,self.num_lbl)).cuda() # kernel matrix of unlabeled vectors
        ...
        self.dict_lbl = self.ART.dict_lbl # dictionary helpful in saving the feature vectors for labeled
        self.dict_unlbl = self.ART.dict_unlbl # dictionary helpful in saving the feature vectors for unlabeled
        ...
        #ART-SS aleotoric uncertainty and rejection
        self.unlbl_sig = self.ART.unlbl_sig#torch.zeros((self.num_unlbl,1)).cuda()
        self.lbl_sig = self.ART.lbl_sig #torch.zeros((self.num_lbl,1)).cuda()
    ...
    def gen_featmaps_unlbl(self,dataloader,net,device):
        self.ART.gen_featmaps_unlbl(dataloader,net,device)
        self.ker_unlbl = self.ART.ker_unlbl
        self.ker_unlbl_ang = self.ART.ker_unlbl_ang
        self.reject_unlabl = self.ART.reject_unlabl
        self.dict_unlbl = self.ART.dict_unlbl
        self.Fz_lbl = self.ART.Fz_lbl
        self.unlbl_sig = self.ART.unlbl_sig
        return

    def gen_featmaps(self,dataloader,net,device):
        self.ART.gen_featmaps(dataloader,net,device)
        self.Fz_lbl = self.ART.Fz_lbl
        self.lbl_sig = self.ART.lbl_sig
        self.thrsh_dist = self.ART.thrsh_dist
        self.thrsh_ang = self.ART.thrsh_ang 
        self.ker_lbl = self.ART.ker_lbl
        self.ker_lbl_ang = self.ART.ker_lbl_ang
        self.dict_lbl = self.ART.dict_lbl
        return
